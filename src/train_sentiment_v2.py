"""
train_sentiment_v2.py
----------------------
Week 2, v2: utterance-level sentiment classifier on TSDAE domain-adapted
...
Pipeline:
  1. Load processed/<SPLIT>/utterance_sentiment.jsonl (LLM-generated sentiment
     ...
     are not gold labels).
  2. Encode utterance text with the TSDAE-adapted model at models/tsdae-adapted/
     (bert-base-uncased base, domain-adapted on the HOPE corpus via
     domain_adaptation.py), so v1 and v2 embeddings are directly comparable.
...
Output:
  models/sentiment-v2/classifier.joblib
  models/sentiment-v2/metrics.json

Usage:
  python src/train_sentiment_v2.py
  python src/train_sentiment_v2.py --batch-size 16
"""
...
OUTPUT_DIR    = os.path.join(REPO_ROOT, "models", "sentiment-v2")
...
def load_embedding_model(model_dir: str) -> SentenceTransformer:
    """
    Loads the TSDAE domain-adapted SentenceTransformer.

    Args:
        model_dir: Path to the saved domain-adapted model
            (output of domain_adaptation.py).
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train v2 (TSDAE domain-adapted) sentiment classifier.")
    parser.add_argument(
        "--batch-size",
        type=int,
        default=BATCH_SIZE,
    ...
    parser.add_argument(
        "--model-dir",
        type=str,
        default=TSDAE_MODEL_DIR,
        help=f"Path to the TSDAE domain-adapted model (default: {TSDAE_MODEL_DIR}).",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    print(f"Loading domain-adapted model from {args.model_dir}...")
    embedder = load_embedding_model(args.model_dir)
    ...
    results = {
        ...
        "domain_adapted": True,
        "domain_adapted_model_dir": args.model_dir,
